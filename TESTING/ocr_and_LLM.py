...
def ocr_gpt(image_path):
        ...
        "messages": [
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": f"""Perform OCR on the given invoice"""
                    },
                    {
                        "type": "image_url",
                        "image_url": {
    ...
    prompt = f"""
    Extract the following invoice attributes from the following text:
    {invoice_data}.
    Format your response as [serial no.]. [key]: [value]. If a value is not present, 
    use 'NULL'. Provide numbers as either integer or float only.
    ...
    4. Seller Information: Full name, address, contact details, and any other information of the seller.
    5. Buyer Information: Full name, address, contact details, and any other information of the buyer.
    6. Purchase Order Number: The buyer's purchase order number.
    7. List of Products/Services: Comma-separated list of all items or services billed. Do not include services like shipping here.
    ...
    21. Bank Information: Seller's bank details for payment.
    22. Invoice Notes: Any additional notes or terms on the invoice.
    23. Shipping Address: The delivery address.
    24. Billing Address: The billing address.
    """
...
ocr_gpt('../uploaded_invoices/IMG_8517.jpeg')
